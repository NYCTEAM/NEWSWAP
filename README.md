...
前台和详情页默认只读取数据库，不会在用户查看时自动扫链。新的买入交易在交易确认后写入数据库。若发生用户关闭页面、网络异常等导致漏记，管理员可以在推荐详情页点击“补扫漏单”，输入管理员密码后再从 Pancake Pair 补扫最近 `BUY_SYNC_BLOCKS` 个区块内、已绑定推荐链接钱包的买入交易。默认约最近 7 天，可按 RPC 能力调大或调小。
